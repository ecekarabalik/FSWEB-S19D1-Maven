...
public class VegetableServiceImpl implements VegetableService {
    ...
    private void validateVegetable(Vegetable vegetable) {
        if (vegetable == null) {
            throw new PlantException("Vegetable cannot be null!", HttpStatus.BAD_REQUEST);
        }
        ...
        if (vegetable.getPrice() == null || vegetable.getPrice() <= 0) {
            throw new PlantException("Vegetable price must be larger than zero!", HttpStatus.BAD_REQUEST);
        }
    }
    ...
    public Vegetable getById(Long id) {
        ...
        return opt.orElseThrow(() ->
                new PlantException("Vegetable not found!", HttpStatus.NOT_FOUND));
    }
